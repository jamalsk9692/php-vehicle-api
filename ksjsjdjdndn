export default async function handler(req, res) {
  ...
  // Get vehicleNumber from query
  const vehicle = req.query.vehicleNumber;
  ...
  try {
    // Make GET request with JSON body using fetch
    const response = await fetch(apiUrl, {
      method: 'POST', // PHP used POST with payload
      headers: {
      ...
      body: JSON.stringify({ vehicleNumber: vehicle })
    });
